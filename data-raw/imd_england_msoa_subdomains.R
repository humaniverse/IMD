...
eimd_msoa_children <- eimd |> aggregate_scores(Score, Children_and_young_people_rank, Children_and_young_people_decile, msoa_code, population)
eimd_msoa_adult    <- eimd |> aggregate_scores(Score, Adult_skills_rank, Adult_skills_decile, msoa_code, population)
eimd_msoa_geo      <- eimd |> aggregate_scores(Score, Geographical_barriers_rank, Geographical_barriers_decile, msoa_code, population)
eimd_msoa_wider    <- eimd |> aggregate_scores(Score, Wider_barriers_rank, Wider_barriers_decile, msoa_code, population)
eimd_msoa_in       <- eimd |> aggregate_scores(Score, Indoors_rank, Indoors_decile, msoa_code, population)
eimd_msoa_out      <- eimd |> aggregate_scores(Score, Outdoors_rank, Outdoors_decile, msoa_code, population)
...
eimd_msoa <-
  eimd_msoa_children  |>
  dplyr::left_join(eimd_msoa_adult, by = "msoa_code") |>
  dplyr::left_join(eimd_msoa_geo,   by = "msoa_code") |>
  dplyr::left_join(eimd_msoa_wider, by = "msoa_code") |>
  dplyr::left_join(eimd_msoa_in,    by = "msoa_code") |>
  dplyr::left_join(eimd_msoa_out,   by = "msoa_code") |>

  # Don't have scores for sub-domains so drop these columns
  dplyr::select(-dplyr::ends_with("Score"))

# Rename
imd_england_msoa_subdomains <- eimd_msoa

# Save output to data/ folder
usethis::use_data(imd_england_msoa_subdomains, overwrite = TRUE)
readr::write_csv(imd_england_msoa_subdomains, "data-raw/imd_england_msoa_subdomains.csv")
