##
## Calculate deprivation in Wards (using 2020 codes) for England and Wales
##
library(tidyverse)
library(readODS)
library(readxl)
...
# Source: https://geoportal.statistics.gov.uk/datasets/lower-layer-super-output-area-2011-to-ward-2020-to-lad-2020-lookup-in-england-and-wales-v2
lsoa_ward = read_csv("https://opendata.arcgis.com/datasets/6408273b5aff4e01ab540a1b1b95b7a7_0.csv")
lsoa_ward = lsoa_ward %>% 
...
eimd_ward = eimd %>% 
  aggregate_scores(domain = "IMD", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")

eimd_ward_income   = eimd %>% aggregate_scores(domain = "Income", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_employ   = eimd %>% aggregate_scores(domain = "Employment", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_edu      = eimd %>% aggregate_scores(domain = "Education", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_health   = eimd %>% aggregate_scores(domain = "Health", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_crime    = eimd %>% aggregate_scores(domain = "Crime", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_barriers = eimd %>% aggregate_scores(domain = "Housing_and_Access", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
eimd_ward_env      = eimd %>% aggregate_scores(domain = "Environment", score_suffix = "_score", rank_suffix = "_rank", decile_suffix = "_decile", aggregate_by = "WD20CD")
...
eimd_ward = eimd_ward %>% 
  left_join(eimd_ward_income,   by = "WD20CD") %>% 
  left_join(eimd_ward_employ,   by = "WD20CD") %>% 
  left_join(eimd_ward_edu,      by = "WD20CD") %>% 
  left_join(eimd_ward_health,   by = "WD20CD") %>% 
  left_join(eimd_ward_crime,    by = "WD20CD") %>% 
  left_join(eimd_ward_barriers, by = "WD20CD") %>% 
  left_join(eimd_ward_env,      by = "WD20CD")

# Save
write_csv(eimd_ward, "data/English IMD - Ward 2020.csv")  
...
# Save
write_csv(wimd_ward, "data/Welsh IMD - Ward 2020.csv")
